...
This command creates a [Lambda layer](https://docs.aws.amazon.com/lambda/latest/dg/configuration-layers.html) for each package passes as argument to the option <OptionAliasesInline options={carlin.options.deployLambdaLayer} option="packages" to="/docs/api-reference/deploy-lambda-layer#packages" />.
...
<CodeBlock className="yaml">{carlin.lambdaLayer.buildspecCommands}</CodeBlock>
...
<CodeBlock className="yaml">{carlin.lambdaLayer.buildspec}</CodeBlock>

The CloudFormation template of the CodeBuild project is:

<Template json={carlin.lambdaLayer.codeBuildProjectTemplate} />
...
<Template json={carlin.lambdaLayer.lambdaLayerTemplate} />

<InnerHTML html={carlin.comments.getPackageLambdaLayerStackName} />
